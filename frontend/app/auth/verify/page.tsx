...
function VerifyInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setStatus("ok");
        if (!j.first_api_key) setTimeout(() => router.push("/dashboard"), 600);
      } catch (e) {
        setStatus("error");
  ...
  if (status === "verifying") return <div className="text-center text-gray-400">Verifying your sign-in link…</div>;
  ...
  if (apiKey) return (
    <div>
      ...
      <p className="mt-2 text-gray-400">Your account is ready. Copy this API key — you won't see it again.</p>
      <pre className="mt-6 p-4 bg-gray-900 border border-accent rounded text-accent break-all">{apiKey}</pre>
      <p className="mt-4 text-sm text-gray-400">Set as <code>TOOLPULSE_API_KEY</code> then:</p>
      <pre className="mt-2">{`pip install toolpulse

from toolpulse import monitor
@monitor(tool_name="my_tool")
async def my_tool(...): ...`}</pre>
      <a href="/dashboard" className="mt-6 inline-block px-5 py-2.5 rounded-md bg-accent text-white">Open dashboard →</a>
    </div>
  );
  return <div className="text-center text-gray-400">Signed in. Redirecting…</div>;
}
